      ...
      <a-form :layout="formLayout" :form="form" v-bind="formItemLayout">
        ...
        <div class="form-row" :class="delinquentDataForUpdate?'read-only-form-data':''">
          ...
          <div class="form-cell">
            ...
            >
              <a-radio-group
                @change="onChangeSearchUserType"
                v-model="userSearchType"
                class="ant-col-10"
                style="max-width:180px;"
              >
                <a-radio-button value="入居者名">入居者名</a-radio-button>
                <a-radio-button value="承認番号">承認番号</a-radio-button>
              </a-radio-group>
              <a-auto-complete
          ...
          <div class="form-cell">
            ...
            >
              <ImageUpload
                ...
                :imageData="contractorOtherFile"
                :imageCbFunc="contractorOtherFileFunc"
              ></ImageUpload>
            </a-form-item>
          </div>
          <div class="form-cell">
...
export default {
  ...
  watch: {
    ...
    searchedUser: {
      handler(userData) {
        console.log("searchedUsersearchedUser", userData);
        if (userData) {
          this.updateUserDatas(userData);
        } else {
          this.clearDatas();
        }
      },
    ...
    delinquentDataForUpdate: {
      handler(delinquentData) {
        ...
        if (delinquentData) {
          ...
          this.nonPayMonthly = delinquentData.nonPayMonthly;
          this.delinquentFee = delinquentData.delinquentFee;
          this.charges = delinquentData.charges;
          this.arrears = delinquentData.arrears;
          this.paymentDate = delinquentData.paymentDate;
  ...
  methods: {
    ...
    updateUserListDataSource(userList) {
      let dataList = [];
      if (this.userSearchType == "入居者名") {
        dataList = userList.map(item => item.contractorName);
      } else if (this.userSearchType == "承認番号") {
    ...
    handleChangeDelinquentList(value) {
      ...
      this.userTypeDataSource = dataList.filter(
        item => item.indexOf(value) != -1
      );
      this.searchedUser = {
        blackList: false,
      ...
      if (this.userTypeDataSource.length == 1) {
        let filteredUser = {};
        if (this.userSearchType == "入居者名") {
          filteredUser = this.userList.filter(
    ...
    onChangePaymentDate(date, dateString) {
